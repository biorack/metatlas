...
def test_get_msms_hits07(metatlas_dataset, msms_refs, mocker):
    mocker.patch("metatlas.plots.dill2plots.load_msms_refs_file", return_value=msms_refs.iloc[0:0])
    hits = dill2plots.get_msms_hits(metatlas_dataset)
    #hits.drop(columns=["ref_frags", "data_frags"], inplace=True)
    assert len(hits) == 0
