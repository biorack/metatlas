...
def test_analysis_identifiers01(sqlite, configuration):
...
def test_analysis_identifiers03(username, sqlite_with_atlas, configuration):
    with pytest.raises(
...
def test_analysis_identifiers04(username, sqlite_with_atlas, configuration):
    with pytest.raises(
        traitlets.traitlets.TraitError,
...
def test_analysis_identifiers05(username, sqlite_with_atlas, configuration):
    with pytest.raises(
        traitlets.traitlets.TraitError,
        match="The 'analysis_number' trait of an AnalysisIdentifiers instance expected an int, not",
    ):
...
def test_analysis_identifiers06(username, sqlite_with_atlas, configuration):
...
def test_analysis_identifiers07(username, sqlite_with_atlas, configuration):
    with pytest.raises(
        traitlets.traitlets.TraitError,
...
def test_analysis_identifiers08(username, sqlite_with_atlas, caplog, mocker, lcmsrun, configuration):
...
def test_analysis_identifiers09(
    sqlite_with_atlas, username, mocker, lcmsrun, groups_controlled_vocab, configuration
):
    mocker.patch("metatlas.plots.dill2plots.get_metatlas_files", return_value=[lcmsrun])
    experiment = "20201106_JGI-AK_PS-KM_505892_OakGall_final_QE-HF_HILICZ_USHXG01583"
    polarity = "positive"
...
def test_analysis_identifiers_atlas01(analysis_ids, username):
    assert (
        analysis_ids.atlas == f"505892_OakGall_final_HILICz150_ANT20190824_PRD_EMA_Unlab_POS_{username}_0_0"
    )
