...
task mzml_file_conversion {
    ...
    command {
        shifter --image=jfroula/mzml-convert:1.0.3 mzml_loader_jaws.py \
                -i ${mzml_file} \
                -o ${h5_name}
    }
    output {
...
task spectral_hits_create_cmds {
    ...
    command {
        shifter --image=jfroula/spectral-hits:1.0.2 spectral_hits_jaws.py -f -n -c ${config} -w spectral_hits_cmds ${mzml_dir}
    }
    output {
...
task spectral_hits_run_cmds {
   ...
   command {
       shifter --image=jfroula/spectral-hits:1.0.2 ${spectral_single_cmd} -c ${config}
   }
   output {
...
task pactolus {
    ...
    command {
        shifter --image=jfroula/pactolus:1.0.10 score_mzmlfile.py \
        --infile ${mzml_file} \
