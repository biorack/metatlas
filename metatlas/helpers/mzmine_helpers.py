...
# def command_line_make_formatted_csv_from_filtered(json_filename=None):
#     """
#     Script only meant to be run at command line to load the filtered mzmine-csv
#     and make a simplified metatlas formatted csv
#     """
#     if not json_filename:
#         json_filename = sys.argv[1]
#     with open(json_filename) as data_file:    
#         params = json.load(data_file)
#     file_to_convert = os.path.join(params['basedir'],'%s_%s_filtered.csv'%(params['basename'],params['polarity']))
...
#         df.to_csv(file_to_convert.replace('.csv','') + '_filtered_formatted.csv',index=True) #save a simplified mzmine-like csv as a backup of all features found.

def make_figures_from_filtered_data(params,all_files,my_atlas):

    pool = mp.Pool(processes=min(64, len(all_files)))
    print("# Made Pool")
...
def clean_and_filter_mzmine_output(json_filename=None,n_peaks=1000):
    ...
    if os.path.isfile(file_to_convert):
        ...
        for my_file in groups.items:
            all_files.append((my_file,groups,atlas_df,my_atlas))
        print("# Made filelist")
        print('# Getting data 1 asdfasdfasdf')
...
def make_targeted_mzmine_job(basedir,basename,polarity,files):
    ...
    task.mz_tolerance = 10.0
    task.min_sn_ratio = 2.0
    task.output_csv = os.path.join(basedir,'%s_%s_filtered.csv'%(basename,task.polarity))
    task.output_workspace = os.path.join(basedir,'%s_%s_filtered.mzmine'%(basename,task.polarity))
    task.input_xml = os.path.join(basedir,'%s_%s_filtered.xml'%(basename,task.polarity))
    
    peak_list_filename = os.path.join(basedir,'%s_%s_formatted_peakfiltered.csv'%(basename,polarity))
    task.mzmine_launcher = get_latest_mzmine_binary()
