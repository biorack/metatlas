...
def yield_ppm(dataset, name, threshold, tolerance, mz_t, rt_max):
    """
    Helper for get_ppm. Given a name and a dataset, returns a filtered and bounded dataset for ppm under name,
...
def get_ppms(dataset, tolerance=15, mz_std=229.981116596, threshold=1e4):
    ...
    """
    # Switch off from pos data to neg data if needed
    ms1 = 'ms1_pos'
    ms2 = 'ms2_pos'
    data = dataset[ms1]
    if data.empty:
    ...
    return (d_1.reset_index(), d_2.reset_index(), d_3.reset_index(), ms1_max_i, rt_max)

# if neg file, then use std_neg
# ms1_pos should be empty
...
def data_verify(file_name):
    ...
    samples_dict['ms2_precursor ppm weighted mean'] = weighted_mean(ppms[1]) #hacky
    ...
    samples_dict['acquisition time'] = datetime.fromtimestamp(samples_dict['acquisition timestamp']).strftime('%Y-%m-%d %H:%M:%S')
...
# Runs the files to create a list of dicts which contain info about
...
def clean_up(samples, files):
    ...
    sorted_cols = ['file name', 'ms1 ppm', 'ms1 ppm weighted mean', 'ms1 intensity', 'ms1 max intensity']
    sorted_cols.extend(['rt', 'rt at max intensity', 'ms2_precursor ppm', 'ms2_precursor ppm weighted mean'])
    sorted_cols.extend(['ms2 ppm', 'ms2 ppm weighted mean', 'acquisition timestamp', 'acquisition time'])

    # get the full report
    df = df.loc[:,sorted_cols]
    df = df.sort_values('file name')
    # This directory should be modular in case we decide to move it
    csv_name = "/project/projectdirs/metatlas/projects/istd_logs/%s_run.csv" % path
    df.to_csv(csv_name, index = False)
    
    print 'begin clean up'
    # abridged report
    df = df.sort_values('file name')
    sorted_cols = ['file name', 'acquisition time', 'ms1 ppm weighted mean', 'ms1 max intensity', 'rt at max intensity']
    sorted_cols.extend(['ms2_precursor ppm weighted mean', 'ms2 ppm weighted mean'])
    df = df.loc[:,sorted_cols]
...
def convert_to_html(df):
    ...
    html_table = html_table.replace('<table border="1"', '<table style="width: 100%;')
    html_table = html_table.replace('<th>', '<th style="border-bottom: 1px solid #ddd; padding: 8px; text-align: left; margin: 0">')
    html_table = html_table.replace('<td>', '<td style="border-bottom: 1px solid #ddd; padding: 8px; text-align: left; margin: 0">')
    html_table = html_table.replace('<tr style="text-align: right;">', '<tr style="text-align: left">')
    return html_table

# make emails more modular
# sends an email of the run(s) done
def send_run_email(username, run_file, tables, standard='ABMBA'):
    ...
    for f in run_file:
        ...
        attachment.add_header("Content-Disposition", "attachment", filename=f.replace('/project/projectdirs/metatlas/projects/istd_logs/', ''))
...
def run_checker():
    ...
    check = 'find %s -name "*.h5" -mmin -%d -size +2k' % (origin_directory, delta)
    valid_files = check_output(check, shell=True).decode('utf-8').splitlines()
    valid_files = set(valid_files)

    files =  defaultdict(set) # dict of user name and the directory of their files
    ...
    for username in files.keys():
        ...
        for directory in files[username]:
            ...
            dir_files = glob.glob(os.path.join(origin_directory + extension,'*.h5'))
            # contains a list of dicts
            samples = process_files(dir_files)
            # a list containing a csv name and a tuple of the non-blank, blank dataframes
